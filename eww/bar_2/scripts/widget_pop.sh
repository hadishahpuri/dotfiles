...
run() {
    ${EWW_BIN} -c $HOME/.config/eww/bar_2 open calendar
}

# Open widgets
if [[ ! -f "$LOCK_FILE" ]]; then
    # ${EWW_BIN} -c $HOME/.config/eww/bar_2 close system music_win audio_ctl
    touch "$LOCK_FILE"
    run && echo "ok good!"
else
    ${EWW_BIN} -c $HOME/.config/eww/bar_2 close calendar
